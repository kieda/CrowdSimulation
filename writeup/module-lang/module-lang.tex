...
\emph{mod} & ::= \emph{javaRef}\;|\;\textsf{parent}(\emph{javaRef}, \emph{modList})\\
...
\inferrule{((\Gamma, \mathcal{C}), (\Gamma', \mathcal{C}')) \in \expr{\emph{package}\;\DOT\;\emph{javaRef}}}
  {\begin{array}{c}
   \mathcal{C} = \mathcal{C}', \\ 
   (\emph{package}\;\DOT\;\emph{javaRef})\in dom(\texttt{ClassLoader}), \\
...
We introduce a utility function, \emph{isParent}, which will help us define the final two rules. This function validates a possible child module in a context. 
...
\qquad \begin{array}{rl}
 & (javaRef \in dom(\Gamma) \wedge \mathcal{C} = \epsilon)\\
\vee & (\emph{javaRef}, \texttt{SubModule<}\tau\texttt{>}) \in \Gamma \wedge \mathcal{C} <: \tau
\end{array}
\end{array}\]
In other words, we first check that our java reference is in our namespace. Then, we ensure that the declared parent of that java reference is an instance of the allowed super types for the module. The final two rules are outlined below,\\
\begin{mathpar}
\inferrule{(\sigma, \sigma') \in \expr{\emph{javaRef}}}
  {\emph{isParent}\;\sigma\;\emph{javaRef}}
